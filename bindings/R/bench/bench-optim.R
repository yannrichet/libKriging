...
Xn_optim = matrix(NA,ncol=2,nrow=0)
Xn_OptimLib = matrix(NA,ncol=2,nrow=0)
...
for (ix0 in 1:nrow(X0)) {
  ...
  Xn_optim = rbind(Xn_optim,o$par)
  
  X = bench_optim(x0)
  points(X[-1,1],X[-1,2],col=rgb(0,0,1,.2),pch=20)
  lines(X[-1,],col=rgb(0,0,1,.2),lty=2)
  points(X[nrow(X),1],X[nrow(X),2],col='blue',pch='x')
  Xn_OptimLib  = rbind(Xn_OptimLib,X[nrow(X),])
}

write.csv(Xn_optim,"Xn_optim.csv")
write.csv(Xn_OptimLib,"Xn_OptimLib.csv")
